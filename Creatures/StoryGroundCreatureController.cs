...
{
    public abstract class StoryGroundCreatureController : OldStoryCreatureController
    {
        public StoryGroundCreatureController(Creature creature, OnlineCreature oc, int playerNumber, SlugcatCustomization customization) : base(creature, oc, playerNumber, customization)
        {
        ...
        public const float zeroGTreshold = 0.3f;
        public readonly bool _wallClimber;
        public bool canZeroGClimb;
        public bool WallClimber => _wallClimber || (creature.room.gravity <= zeroGTreshold && canZeroGClimb);

        public readonly bool _swimWithPathing;
        ...
        public virtual void Jump()
        {
            ...
            {
                RainMeadow.RainMeadow.Debug("super jump");
                superLaunchJump = 0;
                OnJump();
                this.jumpBoost = 6f;
                this.forceBoost = 6;
            ...
            else if (canPoleJump > 0)
            {
                ...
                {
                    RainMeadow.RainMeadow.Debug("beamtip jump");
                    OnJump();
                    this.forceJump = 10;
                    this.jumpBoost = 8f;
                ...
                {
                    RainMeadow.RainMeadow.Debug("horiz beam jump");
                    OnJump();
                    this.forceJump = 10;
                    this.jumpBoost = 8f;
                    flipDirection = this.input[0].x;
                ...
                if (this.input[0].x != 0)
                {
                    RainMeadow.RainMeadow.Debug("pole side jump");
                    OnJump();
                    this.forceJump = 10;
                    flipDirection = this.input[0].x;
                ...
                if (this.input[0].y <= 0)
                {
                    RainMeadow.RainMeadow.Debug("pole drop");
                    OnJump();
            ...
            {
                RainMeadow.RainMeadow.Debug("corridor jump");
                OnJump();
                this.jumpBoost = 6;
            ...
            {
                RainMeadow.RainMeadow.Debug("normal jump");
                OnJump();
                this.jumpBoost = 6;
            ...
            {
                RainMeadow.RainMeadow.Debug("water jump");
                OnJump();
                this.jumpBoost = 8f;
            ...
            else if (canWallJump != 0)
            {
                ...
                {
                    RainMeadow.RainMeadow.Debug("ledge climb"); // against ledge
                    OnJump();
                    this.jumpBoost = 3f;
                ...
                {
                    RainMeadow.RainMeadow.Debug("ledge drag"); // trying to get past ledge
                    OnJump();
                    this.jumpBoost = 3f;
                    // "middle of edge" of air above ledge
                ...
                {
                    RainMeadow.RainMeadow.Debug("wall jump");
                    OnJump();
                    this.jumpBoost = 6f;
                    this.forceBoost = 6;
                    var jumpdir = new Vector2(-Mathf.Sign(canWallJump), 1.74f).normalized;
                    RainMeadow.RainMeadow.Debug("jumpdir " + jumpdir);
                    cs[0].vel.y = 0f;
            ...
            {
                RainMeadow.RainMeadow.Debug("climb jump");
                OnJump();
                this.jumpBoost = 3f;
                var jumpdir = (cs[0].pos - cs[1].pos).normalized + inputDir;
                for (int i = 0; i < cc; i++)
            ...
            {
                RainMeadow.RainMeadow.Debug("Unhandled jump");
            }
        }

        public bool TileAccessible(Room room, IntVector2 pos)
        {
            // skip dumb checks
        ...
        public override bool BaseFindDestination(WorldCoordinate basecoord, out WorldCoordinate toPos, out float magnitude)
        {
            if (base.BaseFindDestination(basecoord, out toPos, out magnitude)) return true;
            ...
            if (localTrace) RainMeadow.RainMeadow.Debug($"moving from {basecoord.Tile} towards {toPos.Tile}");
            ...
            if (this.input[0].y > 0 && !HasFooting)
            {
                ...
                //{
                //    RainMeadow.RainMeadow.Debug("grip!");
            ...
            if (this.input[0].y > 0 && (previousAccessibility <= AItile.Accessibility.Climb || currentTile.WaterSurface) && !currentTile.DeepWater)
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    {
                        if (localTrace) RainMeadow.RainMeadow.Debug("pole close");
                ...
                if (!climbing || inputDir.y > 0.75f) // not found yet, OR pulling the stick hard
                {
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        {
                            if (localTrace) RainMeadow.RainMeadow.Debug("pole far");
            ...
            if (!climbing)
            {
                ...
                if (this.input[0].x != 0) // to sides
                {
                    ...
                    {
                        if (localTrace) RainMeadow.RainMeadow.Debug("down");
                        toPos = WorldCoordinate.AddIntVector(toPos, new IntVector2(0, -1));
                    }
                    ...
                    if (inputDir.magnitude > 0.75f && aheadAccessible)
                    {
                        ...
                        {
                            if (localTrace) RainMeadow.RainMeadow.Debug("reaching further");
                            toPos = furtherOut;
                            magnitude = 1f;
                        }
                        ...
                        {
                            if (localTrace) RainMeadow.RainMeadow.Debug("further up");
                            toPos = WorldCoordinate.AddIntVector(furtherOut, new IntVector2(0, 1));
                            magnitude = 1f;
                        }
                        else if (TileAccessible(room, furtherOut.Tile + new IntVector2(0, -1)) && QuickConnectivity.Check(room, creature.Template, basecoord.Tile, furtherOut.Tile + new IntVector2(0, -1), 6) > -1)
                        {
                            if (localTrace) RainMeadow.RainMeadow.Debug("further down");
                            toPos = WorldCoordinate.AddIntVector(furtherOut, new IntVector2(0, -1));
                            magnitude = 1f;
                        }
                    }
                ...
                {
                    if (localTrace) RainMeadow.RainMeadow.Debug("vertical");
                    ...
                    {
                        if (localTrace) RainMeadow.RainMeadow.Debug("right");
                    ...
                    {
                        if (localTrace) RainMeadow.RainMeadow.Debug("left");
                        toPos = WorldCoordinate.AddIntVector(toPos, new IntVector2(-1, 0));
                    }
                    if (inputDir.magnitude > 0.75f)
                    {
                        ...
                        {
                            if (localTrace) RainMeadow.RainMeadow.Debug("reaching");
                            toPos = furtherOut;
                            magnitude = 1f;
                        }
                    }
            ...
            {
                if (localTrace) RainMeadow.RainMeadow.Debug("pathable");
                return true;
            }
            else
            {
                // no pathing
                if (localTrace) RainMeadow.RainMeadow.Debug("unpathable");
                ...
                {
                    // force movement
                    if (localTrace) RainMeadow.RainMeadow.Debug("forced move to " + toPos.Tile);
                    magnitude = 1f;
                    this.MovementOverride(new MovementConnection(MovementConnection.MovementType.Standard, basecoord, toPos, 2));
                    return true;
                }
                ...
                {
                    if (localTrace) RainMeadow.RainMeadow.Debug("unable to move");
                    return false;
                }
            }
        }
        ...
        public override void ConsciousUpdate()
        {
            ...
            if (HasFooting)
            {
                if (IsOnCorridor)
                {
                    if (localTrace) RainMeadow.RainMeadow.Debug("can corridor boost");
                    this.canCorridorBoost = 5;
                }
                else if (IsOnGround)
                {
                    if (localTrace) RainMeadow.RainMeadow.Debug("can ground jump");
                    this.canGroundJump = 5;
                }
                else if (IsOnPole)
                {
                    if (localTrace) RainMeadow.RainMeadow.Debug("can pole jump");
                    this.canPoleJump = 5;
                }
                else if (IsOnClimb)
                {
                    if (localTrace) RainMeadow.RainMeadow.Debug("can climb jump");
                    this.canClimbJump = 5;
                }
            }
            ...
            {
                if (localTrace) RainMeadow.RainMeadow.Debug("can walljump");
                this.canWallJump = 5 * wallValue;
            }
            if (IsOnWaterSurface && canWaterJump >= 0) // negative = locked
            {
                if (localTrace) RainMeadow.RainMeadow.Debug("can water jump");
                this.canWaterJump = 5;
            }
            ...
            if (this.CanPounce)
            {
                ...
                {
                    if (localTrace) RainMeadow.RainMeadow.Debug("charging pounce");
                    this.wantToJump = 0;
            ...
            {
                if (localTrace) RainMeadow.RainMeadow.Debug("jumping");
                this.Jump();
                this.wantToJump = 0;
            }
            ...
            if (scd.moveSpeed > 0f && inputDir != Vector2.zero)
            {
                ...
                if (!HasFooting && creature.room.gravity > zeroGTreshold && creature.mainBodyChunk.submersion < 0.5f)
                {
                    ...
                    if (amount > 0f)
                    {
                        mainchunk.vel.x += amount * inputDir.x * scd.moveSpeed;
                    }
                }
                else
                {
                    ...
                    if (amount > 0f)
                    {
                        mainchunk.vel += amount * inputDir * scd.moveSpeed;
                    }
                }
            }
        ...
        public abstract void GripPole(Room.Tile tile0);
        public abstract void ClearMovementOverride();
        public abstract void MovementOverride(MovementConnection movementConnection);
